...
function IncomeWorkspace({ storageKey }: { storageKey: string }) {
  ...
  const dividendLines = stocks.map(symbol => {
    ...
    else if (references[symbol]?.data?.method === 'accumulating') line.reason = 'ETF ad accumulazione: proventi reinvestiti nel fondo, nessun dividendo in contanti';
    else if (references[symbol]?.data?.method === 'non_distributing') line.reason = 'ETC sull’oro: l’emittente conferma che non distribuisce proventi';
    else if (references[symbol]?.data?.method === 'no_payments_last_year') line.reason = 'La fonte non riporta dividendi negli ultimi 12 mesi';
    else if (references[symbol]?.data?.method === 'no_current_distribution') line.reason = 'La fonte non segnala dividendi correnti';
    else if (references[symbol]?.data?.method === 'confirmed_trailing_12m') line.reason = 'Dividendi confermati degli ultimi 12 mesi, rettificati per i frazionamenti. Previsioni escluse';
    else if (references[symbol]?.data?.status === 'no_history') line.reason = 'Non risultano distribuzioni nello storico della fonte';
    return line;
  });
  const blockedReferences = stocks.filter(symbol => ['upstream_access_denied', 'upstream_rate_limited'].includes(references[symbol]?.code ?? '')).length;
  ...
  return <form ref={form} noValidate onInput={() => { setFormEdited(true); setSaveMessage(''); }} onSubmit={e => { e.preventDefault(); save(); }} className="space-y-6 pb-8">
    ...
    {blockedReferences > 0 && <div role="status" className="rounded-xl border border-warning/30 bg-warning/5 p-4 text-sm text-text-1"><p className="font-medium text-text-0">Collegamento ai dividendi bloccato per {blockedReferences} titoli</p><p className="mt-1">La fonte esterna sta rifiutando o limitando le richieste del server. Il recupero completo richiede una fonte accessibile. Il totale attuale include solo le rendite disponibili; la quota mancante resta da determinare.</p></div>}
    ...
    {tab === 'dati' && <>
      <Panel title="Dividendi: recupero automatico"><p className="mb-5 text-sm text-text-1">Sommiamo i dividendi confermati con stacco negli ultimi 12 mesi e li moltiplichiamo per le quote eleggibili attuali, incluse quelle nei portafogli copiati. Le previsioni della fonte sono escluse. La media mensile indica una rendita lorda al ritmo storico: gli importi futuri possono cambiare e gli accrediti eToro dipendono dalle quote possedute alle date di stacco, dalle ritenute e dal cambio.</p>{!stocks.length ? <p className="text-sm text-text-2">I titoli compariranno automaticamente dopo il caricamento del portafoglio.</p> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs text-text-2"><tr><th className="pb-3 pr-4">Titolo</th><th className="pb-3 pr-4">Dividendo annuo / quota</th><th className="pb-3 pr-4">Media mensile stimata</th><th className="pb-3">Fonte e aggiornamento</th></tr></thead><tbody className="divide-y divide-hairline">{dividendLines.map(line => { const data = references[line.label]?.data; const rule = dividendRule(line.label); return <tr key={line.id}><td className="py-4 pr-4 align-top font-medium text-text-0">{line.label}</td><td className="py-4 pr-4 align-top text-text-1">{rule.annualPerShare === null ? line.annualGross === 0 ? 'Non applicabile' : refBusy ? 'In recupero' : 'Non disponibile' : `${num(rule.annualPerShare)} ${rule.currency}`}<p className="mt-1 max-w-xs text-xs text-text-2">{line.reason}</p></td><td className="py-4 pr-4 align-top text-text-0">{money(annualAmount(line) === null ? null : annualAmount(line)! / 12)}</td><td className="py-4 align-top text-xs text-text-2">{data ? <><a className="text-gain underline" href={data.source} target="_blank" rel="noreferrer">{data.provider}</a><p className="mt-1">Letto {new Date(data.asOf).toLocaleString('it-IT')}</p>{data.sourceUpdatedAt && <p>Dati fonte: {data.sourceUpdatedAt}</p>}</> : line.annualGross === 0 && rule.annualPerShare === null ? <>Posizione esclusa dal calcolo</> : rule.source ? <>Dato salvato in precedenza · {rule.source}</> : references[line.label]?.code === 'upstream_access_denied' ? <>Accesso della fonte bloccato</> : references[line.label]?.code === 'upstream_rate_limited' ? <>Richieste limitate dalla fonte</> : <>Fonte temporaneamente non disponibile. Usa Aggiorna per riprovare.</>}</td></tr>; })}</tbody></table></div>}<p className="mt-4 text-xs text-text-2">Una fonte non disponibile rimane esclusa dal totale e viene segnalata: non è convertita in un dividendo di zero. Il recupero dipende dall’accessibilità delle fonti esterne.</p></Panel>
